...
class Helpers {
    ...
    // Function to extract natural sucrose sources
   private func extractNaturalSucrose(from ingredients: String) -> [String] {
    ...
    //    let sanitizedIngredients = ingredients
    ...
    //        .components(separatedBy: ", ")
        ...
        for ingredient in sanitizedIngredients {
            if naturalSucroseSources.contains(where: { ingredient.contains($0) }) {
                naturalSucroseMatches.insert(ingredient)
            }
        }
        return Array(naturalSucroseMatches).filter { !$0.contains("oil") }
    }
    ...
    func findSucroseIngredients(in ingredients: String) -> [[String]] {
        ...
        let naturalSucrose = extractNaturalSucrose(from: replacedIngredients)
        categorizedIngredients.append(naturalSucrose) // Append natural sucrose to the result
        return categorizedIngredients
    }
}
